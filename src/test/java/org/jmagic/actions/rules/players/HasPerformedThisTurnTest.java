...
class HasPerformedThisTurnTest {
    ...
    void testRule() {
        ...
        rule = new HasPerformedThisTurn(AdvanceGame.class);
        ...
        rule = new HasPerformedThisTurn(AdvanceGame.class, players.get(0));
        assertTrue(rule.isValid(advancedState));
        assertFalse(rule.isValid(initialState));


        // This player should have not performed any actions so far.
        rule = new HasPerformedThisTurn(AdvanceGame.class, players.get(1));
        ...
        assertThrows(NoSuchElementException.class,
            () -> new HasPerformedThisTurn(AdvanceGame.class, p).isValid(initialState));
        assertThrows(NoSuchElementException.class,
            () -> new HasPerformedThisTurn(AdvanceGame.class, p).isValid(advancedState));
    }
    ...
    void testToString() {
        ...
        r = new HasPerformedThisTurn(AdvanceGame.class);
        assertEquals("HasPerformedThisTurn(AdvanceGame, active)", r.toString());

        r = new HasPerformedThisTurn(AdvanceGame.class, players.get(0));
        assertEquals("HasPerformedThisTurn(AdvanceGame, test-player-1)", r.toString());
    }
